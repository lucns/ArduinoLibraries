...
void PubSubClient::setWifi(Client& client) {
    this->wifi = &client;
}
...
bool PubSubClient::connect(const char *id, const char *user, const char *pass, const char* willTopic, uint8_t willQos, bool willRetain, const char* willMessage, bool cleanSession) {
    if (!connected()) {
        ...
        if(wifi->connected()) {
            ...
            if (domain != NULL) {
                result = wifi->connect(this->domain, this->port);
            } else {
                result = wifi->connect(this->ip, this->port);
            }
        }
                ...
                if (type == MQTTPUBLISH) {
                    if (callback) {
                        ...
                        if ((this->buffer[0]&0x06) == MQTTQOS1) {
							...
							String message;
							for (int i = 0; i < len-llen-3-tl-2; i++) message += (char)payload[i];
                            callback(topic2, message);
							
                            this->buffer[0] = MQTTPUBACK;
                            this->buffer[1] = 2;
                            this->buffer[2] = (msgId >> 8);
                            this->buffer[3] = (msgId & 0xFF);
                            wifi->write(this->buffer,4);
                            lastOutActivity = t;
							...
							String topic2 = String(topic);
							String message;
							for (int i = 0; i < len-llen-3-tl; i++) message += (char)payload[i];
                            callback(topic2, message);						
                        }
                    }
...
bool PubSubClient::publish_P(const char* topic, const uint8_t* payload, unsigned int plength, bool retained) {
    ...
    rc += wifi->write(this->buffer,pos);

    for (i=0;i<plength;i++) {
        rc += wifi->write((char)pgm_read_byte_near(payload + i));
    }
...
bool PubSubClient::beginPublish(const char* topic, unsigned int plength, bool retained) {
    if (connected()) {
        ...
        uint16_t rc = wifi->write(this->buffer+(MQTT_MAX_HEADER_SIZE-hlen),length-(MQTT_MAX_HEADER_SIZE-hlen));
        lastOutActivity = millis();
...
bool PubSubClient::write(uint8_t header, uint8_t* buf, uint16_t length) {
    ...
    while((bytesRemaining > 0) && result) {
        ...
        rc = wifi->write(writeBuf,bytesToWrite);
    ...
    return result;
#else
    rc = wifi->write(buf+(MQTT_MAX_HEADER_SIZE-hlen),length+hlen);
    lastOutActivity = millis();
...
void PubSubClient::setCallback(MQTT_CALLBACK_SIGNATURE) {
